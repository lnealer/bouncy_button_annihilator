...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
int EXTI1_Triggered = 0;
int button_state = 0; // 0 = not pushed
...
int main(void)
{
  ...
  while (1)
  {
	...
	if (handle_button_trigger())
	    {
	      HAL_GPIO_TogglePin(GPIOA, LED_Pin);
		  EXTI1_Triggered = 0;
	    }

	    //      if ((current_time - last_trigger_time) > 200) { // debounce
	    //          if (!HAL_GPIO_ReadPin(GPIOA, BUTTON_Pin)) { // falling edge
	    //
	    //          }
...
int  handle_button_trigger() {
	...
	if (!button_state) { // first push detected
	...
	if (time_diff > 10 && !HAL_GPIO_ReadPin(GPIOA, BUTTON_Pin)) {
		button_state = 0;
		return 1;
	}
...
//      if ((current_time - last_trigger_time) < 50) { // debounce
